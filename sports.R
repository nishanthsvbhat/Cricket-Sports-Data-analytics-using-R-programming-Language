...
# Read the CSV file
df<- read.csv("D:/R/dream11.csv/dream11.csv")
print(df)
...
#line graph after filtering the data
df1<-df%>%filter(Credits>8.5)
ggplot(df1, aes(x = Player, y = Credits, group = Team, color = Team)) +
  geom_line() +
  labs(title = "Player Credits by Team", x = "Player", y = "Credits") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
